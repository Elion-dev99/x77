...
export function StudioPage() {
  ...
  const { localVideoRef, isLive, error, startBroadcast, stopBroadcast } = useBroadcaster(send, onMessage);
  ...
  const handleCreateAndGoLive = async () => {
    ...
    try {
      const { stream: s } = await api.createStream(form);
      const { stream: live } = await api.goLive(s.id);
      setStream(live);
      await startBroadcast();
      if (connected) {
        send({ type: 'join', streamId: s.id, role: 'broadcaster' });
      }
      setStep('live');
    } catch (err) {
          ...
          <div className="max-w-xl glass rounded-2xl p-8">
            ...
            <div className="space-y-4">
              ...
              <button
                ...
                className="w-full py-3 rounded-xl bg-gradient-to-r from-red-500 to-brand-500 text-white font-semibold flex items-center justify-center gap-2 disabled:opacity-50 transition-opacity"
              >
              ...
              <div className="relative aspect-video bg-black rounded-xl overflow-hidden">
                ...
                <div className="absolute bottom-3 left-1/2 -translate-x-1/2 flex gap-3">
                  <button
                    ...
                    className={`p-3 rounded-full ${audioEnabled ? 'bg-surface-600/80' : 'bg-red-500/80'} text-white transition-colors`}
                  >
                    {audioEnabled ? <Mic className="w-5 h-5" /> : <MicOff className="w-5 h-5" />}
                  </button>
                  <button
                    onClick={() => setVideoEnabled(!videoEnabled)}
                    className={`p-3 rounded-full ${videoEnabled ? 'bg-surface-600/80' : 'bg-red-500/80'} text-white transition-colors`}
                  >
                    {videoEnabled ? <Video className="w-5 h-5" /> : <VideoOff className="w-5 h-5" />}
                  </button>
                  <button
                    onClick={handleEndStream}
                    className="px-6 py-3 rounded-full bg-red-600 text-white font-semibold text-sm hover:bg-red-500 transition-colors"
                  >
                    {t('end_stream')}
                  </button>
                </div>
